...
class StateMachineInstanceTest {
    ...
    @BeforeEach void setUp() {
        machine = new StateMachine<>("test-machine", "v1",
            List.of(new State<Context>("start", ctx -> {}), new State<Context>("end", ctx -> {})),
            List.of(new Transition<>("start", "end", ctx -> true)),
    ...
    @Test void newInstance_isRunningWithInitialState() {
        var instance = new StateMachineInstance(instanceId, machine, BusinessId.ofNullable("biz-001"));
    ...
    @Test void transitionTo_validState_updatesCurrentState() {
        var instance = new StateMachineInstance(instanceId, machine, BusinessId.ofNullable(null));
