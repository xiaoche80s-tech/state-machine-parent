...
public class SnapshotRepository {
    ...
    public String save(String instanceId, String stateName, String inputJson, String outputJson, String status, String errorMessage, int attempt) {
        String id = UUID.randomUUID().toString();
        jdbcTemplate.update(
            "INSERT INTO state_machine_snapshots (id, instance_id, state_name, input, output, status, error_message, attempt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            ps -> {
                int i = 1;
                ps.setString(i++, id);
    ...
    private RowMapper<SnapshotRecord> rowMapper() {
        return (rs, rowNum) -> new SnapshotRecord(
            ...
            rs.getString("error_message"), rs.getInt("attempt"), rs.getTimestamp("executed_at").toInstant());
    }

    public record SnapshotRecord(String id, String instanceId, String stateName,
            String inputJson, String outputJson, String status, String errorMessage, int attempt, java.time.Instant executedAt) {}
}
