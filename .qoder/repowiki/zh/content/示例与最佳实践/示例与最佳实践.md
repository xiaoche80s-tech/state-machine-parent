...
## 更新摘要
**变更内容**
- 更新演示应用上下文路径从 `/state-machine-demo` 到 `/state-machine-demo3`
- 更新启动器模块版本从 `1.0.1-jdk8.release` 到 `1.0.2-jdk8.release`
- 更新 JDK 8 兼容性相关信息
- 更新控制台访问路径和配置说明
...
**更新** 项目现已升级到 `1.0.2-jdk8.release` 版本，提供完整的 JDK 8 兼容性支持。
...
- 上下文与动作/条件
  - Context 提供键值存储；Action/Condition 描述步骤逻辑与路由条件
- 控制台与管理
...
Client->>Controller : POST /state-machine-demo3/demo/order
...
**更新** 所有 API 端点现在使用 `/state-machine-demo3` 上下文路径。
...
## 结论
该演示应用以清晰的分层与可复用的状态机构建器为基础，提供了订单与出库两条完整业务链路。通过挂起/恢复、重试策略与可视化控制台，实现了可观测、可维护、可扩展的状态机执行体系。**更新** 现已升级到 `1.0.2-jdk8.release` 版本，提供完整的 JDK 8 兼容性支持。建议在生产环境中强化访问控制、敏感数据保护与资源池调优，并结合测试用例保障复杂条件与异常分支的正确性。
...
- 版本信息
  - 启动器模块版本：`1.0.2-jdk8.release`
  - 示例应用依赖版本：`1.0.1-jdk8.release`
  - JDK 版本：`1.8`
...
**更新** 所有 API 端点现在使用 `/state-machine-demo3` 上下文路径，控制台访问地址为 `http://localhost:8081/state-machine-demo3/statemachine/`。